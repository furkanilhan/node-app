  ...
  private async handleRegister(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      parseRequestBody(req, async (buffer) => {
        ...
        if (existingUser) {
          return this.sendResponse(res, HTTP_STATUS.BAD_REQUEST, { error: 'Username already exists' });
        }

        const hashedPassword = await bcrypt.hash(password, 10);
        await userService.createUser(username, hashedPassword);

        this.sendResponse(res, HTTP_STATUS.CREATED, { message: 'User registered successfully' });
      });
    } catch (error) {
  ...
  private async handleLogin(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      parseRequestBody(req, async (buffer) => {
        ...
        if (!user) {
          return this.sendResponse(res, HTTP_STATUS.NOT_FOUND, { error: 'User not found' });
        }

        const isPasswordValid = await bcrypt.compare(password, user.password);
        if (!isPasswordValid) {
          return this.sendResponse(res, HTTP_STATUS.UNAUTHORIZED, { error: 'Invalid password' });
        }
